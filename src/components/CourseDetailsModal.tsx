		...
		<div className="modal-overlay" onClick={onClose}>
			<div className="modal-content max-w-md" onClick={(e) => e.stopPropagation()}>
				...
				<div className="flex items-start gap-3 mb-4">
					<div className="platform-chip-bullet mt-1" style={{ "--chip-color": LINE_COLORS[colorIndex] || LINE_COLORS[0] } as CSSProperties} />
					<div className="flex-1 min-w-0">
						<h2 className="font-display text-lg font-semibold">{courseCode}</h2>
				...
				<div className="grid grid-cols-2 gap-3 mb-4">
					{/* Future Classes */}
					<div className="p-3 rounded-xl bg-overlay border border-border">
						<p className="font-mono text-2xl font-bold text-foreground">{futureClasses.length}</p>
						<p className="text-xs text-text-tertiary">Future {futureClasses.length === 1 ? "Class" : "Classes"}</p>
					</div>

					{/* Future Hours */}
					<div className="p-3 rounded-xl bg-overlay border border-border">
						<p className="font-mono text-2xl font-bold text-foreground">{futureHours}</p>
						<p className="text-xs text-text-tertiary">Hours Remaining</p>
					</div>
				</div>

				{/* Summary Row */}
				<div className="flex items-center justify-between p-2 rounded-lg bg-overlay">
					...
					<div className="mt-4">
						...
						<div className="space-y-2 max-h-48 overflow-y-auto">
							{futureClasses.slice(0, 5).map((event, index) => (
								<div key={index} className="flex items-center justify-between py-2 px-3 rounded-lg bg-overlay">
									<div>
